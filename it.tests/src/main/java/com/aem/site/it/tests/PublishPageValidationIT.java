...
public class PublishPageValidationIT {
    ...
    // list files which do return a zero byte response body
    private static final List<String> ZERO_BYTE_FILES = List.of();
    ...
    private static void verifyLinkedResources(HtmlUnitClient client, String path) throws ClientException, IOException, URISyntaxException {
        ...
        assertTrue(path + " does not contain any references!", !references.isEmpty());
        for (URI ref : references ) {
            if (isSameOrigin(client.getUrl(), ref)) {
                ...
                if (ZERO_BYTE_FILES.stream().noneMatch(s -> ref.getPath().startsWith(s))) {
                    if (responseSize == 0) {
                        LOG.warn("Empty response body from [{}], please validate if this is correct", ref.getPath());
                    }
                }

            } else {
                LOG.info("skipping linked resource from another domain {}", ref);
            }
        }
    }
